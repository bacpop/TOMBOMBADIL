#install.packages("cmdstanr", repos = c('https://stan-dev.r-universe.dev', getOption("repos"))) # this does not work on the cluster. Start R, run this command, and choose mirror manually.
library(cmdstanr)
...
for (i in 1:data_list$gene_length) {
  ...
  model_fit <- model$sample(data = data_list_singlelocus, threads_per_chain = 1, parallel_chains = 1) 
...
saveRDS(results_df, file = paste0("../results_cluster/", gene_name, ".rds"))
