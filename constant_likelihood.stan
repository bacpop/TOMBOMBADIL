...
vector likelihood_constant_NY98(vector shared_vec, vector diff_vec,
...
vector likelihood_constant_GTR(vector shared_vec, vector diff_vec,
data array[] real x_r, data array[] int x_i){
  // Get size of shard from last element of x_i
  int n = num_elements (x_i);
  int n_sites = x_i[n];
  ...
  // Reconstruct parameters shared between shards
  real theta = shared_vec[1];
  real alpha = shared_vec[2];
  real beta = shared_vec[3];
  real gamma = shared_vec[4];
  real delta = shared_vec[5];
  real epsilon = shared_vec[6];
  real eta = shared_vec[7];
  real omega = diff_vec[1];
